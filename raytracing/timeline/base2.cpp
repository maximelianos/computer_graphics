...
public:
	...
	BVH_node(vector< shared_ptr<Hittable> > &objects, int beg_idx, int end_idx) {
		int axis = counter;
		counter = (counter + 1) % 3;
		...
		if (len == 1) {
			...
			left = make_shared<BVH_node>(objects, beg_idx, mid);
			right = make_shared<BVH_node>(objects, mid, end_idx);
...
void addcube(HittableList &objects, vec shift_z, material *mat) {
    ...
    addobj(objects, a, b, c, mat->clone());
    addobj(objects, a, c, d, mat->clone());
    
    addobj(objects, g, f, e, mat->clone());
    addobj(objects, h, g, e, mat->clone());
    
    addobj(objects, d, c, g, mat->clone());
    addobj(objects, d, g, h, mat->clone());
    
    addobj(objects, f, b, a, mat->clone());
    addobj(objects, f, a, e, mat->clone());
    
    addobj(objects, d, h, a, mat->clone());
    addobj(objects, h, e, a, mat->clone());
    
    addobj(objects, c, b, f, mat->clone());
    addobj(objects, c, f, g, mat->clone());
}

void read_polygon(HittableList &objects, const char *filename) {
	vector< shared_ptr<Hittable> > tri_list;
	
	// Polygon is represented by upper and bottom lines
	// Then make triangle shapes with them and construct Bounding Volume tree
	vector<vec> upper;
	vector<vec> bottom;
	...
	int bottom_idx = 0;
	int change = 0;
	while (upper_idx + 1 < upper.size() || bottom_idx + 1 < bottom.size()) {
		if (change && upper_idx + 1 < upper.size()) {
			tri_list.push_back( make_shared<Triangle>(upper[upper_idx], upper[upper_idx + 1],
					bottom[bottom_idx], mat->clone()) );
		} else if (!change && bottom_idx + 1 < bottom.size()) {
			tri_list.push_back( make_shared<Triangle>(upper[upper_idx], upper[upper_idx + 1],
					bottom[bottom_idx], mat->clone()) );
		}
		change ^= 1;
...
{	
    ...
    //read_polygon(objects, "letter0.in");
    //return 0;
    ...
    material *clear_glass = new material(MAT_DIELECTRIC, 1.6);
    clear_glass->k_diffuse = 0.7;
    clear_glass->k_mirror = 0.9;
    clear_glass->k_reflect = 0.2;
    
    for (int i = 0; i < 1; i++) {
		vec a(random_double() - 0.5, random_double() * 0.1 - 3.4, -1);
		objects.add( make_shared<Sphere>(a, random_double() + 0.3, clear_glass->clone()) );
	}
    ...
    for (int i = 0; i < 10000; i++) {
		...
		vec b = a + vec(0.5, random_double() * 0.5 - 0.25, -1) * random_double() + vec(0.2, 0.3, 0.05);
		vec c = a + vec(0.2, random_double() * 0.5 - 0.25, -1) * random_double() + vec(0.2, 0.1, -1);
		vec shf = vec(0, 0, -i / 5.0);
		material *tri = new material(MAT_DIFFUSE);
		tri->ambient_color = vec(random_double(),random_double(),random_double());
		objects.add( make_shared<Triangle>(a + shf, b + shf, c + shf, tri) );
	}
	
	//objects.add( make_shared<Circle>(vec(0, 0, -0.2), 0.1, vec(1, 0, -1), vec(0, 1, 0), red_dif->clone()) );
    
    addcube(objects, vec(-1, -3, -4), red_metal);
    addcube(objects, vec(-3.2, -3, -4), red_metal);
    addcube(objects, vec(2.2, -3, -4), red_metal);
    addcube(objects, vec(6, 1, -4), red_metal);
    
    //addcube(objects, vec(-1, -1, 1), red_metal);
    
    
    objects.add( make_shared<InfinitePlane>(vec(0, -4, 0), vec(1, 0, 0), vec(0, 0, -1), red_dif->clone()) );
